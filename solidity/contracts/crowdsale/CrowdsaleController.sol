...
contract ICOContract is usingOraclize, Owned {
    ...
    event TokensBought(uint256 tokens, uint256 price);
    event PriceUpdated(uint256 ethPerToken);

    function ICOContract(address _tokenContractAddress) {
    ...
    function __callback(bytes32 myid, string result, bytes proof) {
        ...
        strings.slice memory decimalValSlice = strings.rsplit(wholeNumberSlice, sepSlice);
        ...
        uint256 actualValue = (wholeUint * 1 ether) + ((decimalUint * 1 ether) / (10**(strings.len(decimalValSlice))));
    ...
    function getDiscountForUser() internal view returns (uint256) {
    ...
    // Returns true if block.timestamp is between Jan 25, 2019 and Jun 27, 2019
    function isCrowdsaleOpen() public view returns (bool) {
        return (block.timestamp >= 1548374400 && block.timestamp < 1561679999);
    }

    // Returns an amount of tokens equivalent to ETH value paid
    function buyToken() public payable {
        require(isCrowdsaleOpen());
        require(msg.value > 0);

        uint256 discountedTokenPerETH = (tokenPerETH * 43 ether) / (43 * (1 ether - getDiscountForUser()));
        uint256 tokens = (msg.value * discountedTokenPerETH) / 1 ether;
        TokensBought(tokens, discountedTokenPerETH);
    ...
    function burnICOTokens(uint256 tokens) public onlyOwner {
        ERC20Interface tokenContract = ERC20Interface(tokenContractAddress);
        tokenContract.burn(tokens);
    }

    // Return all the contracts balance to owner's address
    function getInvestmentsToWallet() public onlyOwner {
